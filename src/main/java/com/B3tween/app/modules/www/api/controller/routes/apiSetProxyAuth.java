            ...
            if (jwtRepository.validateJWT(request)) {
                if (jwtRepository.isUserAdministrator(request)) {
                    try {
                        ...
                        Boolean proxyAuth = Boolean.parseBoolean(json.getString("state").trim());
                        // Get user DTO
                        ...
                        // Set auth
                        if (proxyAuth) {
                            globalRuntime.PROXY_AUTHENTICATION = true;
                            apiUtils.responses.twoHundredOk(clientSocket, origin);
                            return;
                        }
                        globalRuntime.PROXY_AUTHENTICATION = false;
                        // Send response
                        apiUtils.responses.twoHundredOk(clientSocket, origin);
